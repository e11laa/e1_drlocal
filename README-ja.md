...
uv sync
```
*※ Linux/macOS ユーザーの方へ: もし `uv sync` 中に解決エラー（resolution error）が出る場合は、`pyproject.toml` 内の `tool.uv.required-environments` をご自身のプラットフォームに合わせて書き換えるか、そのセクションを削除してください。*
...
また、`src/e1_drlocal/constants.py` を編集することで、使用するモデルを環境に合わせて自由にカスタマイズすることも可能です。
...
**これでセットアップは完了です！** [使い方 (Usage)](#使い方-usage) セクションに戻って、リサーチを開始しましょう。
